...
#[test]
fn mentions_invalid_user_ids_are_ignored() {
    let json_data = json!({
        "body": "hi",
        "msgtype": "m.text",
        "m.mentions": {
            "user_ids": ["@alice:example.org", "not-a-user-id", "@bob:example.org"],
            "room": true,
        },
    });
    ...
    assert_eq!(
        mentions.user_ids,
        [owned_user_id!("@alice:example.org"), owned_user_id!("@bob:example.org")].into()
    );
    assert!(mentions.room);
}

#[test]
fn mentions_all_invalid_user_ids_become_empty() {
    let json_data = json!({
        "body": "hi",
        "msgtype": "m.text",
        "m.mentions": {
            "user_ids": ["not-a-user-id"],
        },
    });
